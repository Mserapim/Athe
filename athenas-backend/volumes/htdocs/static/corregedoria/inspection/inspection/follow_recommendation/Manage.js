Ext._define('corregedoria.inspection.inspection.follow_recommendation.Manage', {
  extend: 'toolkit.widget.TabPanel',

    autoSelectionEmployee: function() {
      if(this.employee_pk === undefined) {
          var rest = Ext._create('corregedoria.inspection.inspection.follow_recommendation.Restful');
          ...
          rest.doRequest(
              rest.getRoute('employee_initial', false, 'GET', {
                  ...
                  success: function(xhr) {
                      ...
                      if(rst.success) {
                          this.workplaces = rst.workplaces;
                          this.getGridInspection().setFilterProperty('in_member_organ__employee', rst.employee_pk, 100, false);
                          this.getGridInspection().setFilterProperty('execution_organ__pk__in', this.workplaces, 100);
                      }
                      else
                          Ext.Msg.show({
    ...
    getGridInspection: function() {
        if(!this._gridInspection) {
            this._gridInspection = Ext._create('corregedoria.inspection.inspection.Grid', {
                ...
                configOrderToolBar: ['viewReport', '->', '-', 'search'],
                hideColumns: ['employee_unicode'],
                sm: new Ext.grid.RowSelectionModel({singleSelect:true}),
                doubleClickHandler: function() { },
                // actionColumn: false,
            });
            this.getGridInspection().setFilterProperty('finalized', false, 101, false);
            this.getGridInspection().setFilterProperty('communicated_organ_execution', true, 102, false);
    ...
    getGridRecommendations: function() {
        if(!this._gridRecommendations) {
            this._gridRecommendations = Ext._create('corregedoria.inspection.inspection.follow_recommendation.Grid', {
                region: 'center',
    ...
    constructor: function(cfg) {
        ...
        corregedoria.inspection.inspection.follow_recommendation.Manage.superclass.constructor.call(this, cfg);
