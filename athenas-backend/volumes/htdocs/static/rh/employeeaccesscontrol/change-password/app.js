...
const fnSubmit = (e) => {
  ...
  else if (sumStrongPassword(data.new_password) < 0.6)
    console.log('bad reputation of password');
  else {
    const req = new Request('/athenas/EACEmployee/change_password/', {
      method: 'POST',
...
const fnMatchPassword = (e) => {
  ...
  const base = document.querySelector('#new_password').value;
  const el = document.querySelector('#matchOutput');
