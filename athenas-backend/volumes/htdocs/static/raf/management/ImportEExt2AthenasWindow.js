...
Ext._define('raf.management.ImportEExt2AthenasWindow', {
    ...
    importarEExt: function() {
        var values = this.getValues();
        var mask = new Ext.LoadMask(this.getEl(), {msg: 'Importação de documento(s) do e-Ext...'});
        if (values.validate) {
            mask.show();
            Ext.Ajax.request({
                scope: this,
                url: core.callAction('RAFFunctionalActivityReport', 'importEExt2AthenasRAF'),
                callback: function() {
                    mask.hide();
                },
                success: function(request) {
                    ...
                    Ext.Msg.show({
                        title: 'Importação de documento(s) do e-Ext',
                        msg: rst.message,
                        icon: Ext.Msg.INFO,
                        buttons: Ext.Msg.OK
                    });
            ...
            this._buttons = [
                {
                    text: '<b>Importar</b>',
                    scope: this,
                    handler: function() { this.importarEExt(); }
                },
                {
                    text: 'Fechar',
                    scope: this,
    ...
    constructor: function(cfg) {
        ...
        Ext.applyIf(
            cfg,
            {
                title: 'Importação de documento(s) do e-Ext/e-Ext',
                modal: true,
                resizable: false,
                border: false,
                width: 700,
        ...
        raf.management.ImportEExt2AthenasWindow.superclass.constructor.call(this, cfg);
    }
});
