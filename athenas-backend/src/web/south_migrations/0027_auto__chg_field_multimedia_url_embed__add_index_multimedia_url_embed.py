...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        ...
        # Changing field 'Multimedia.url_embed'
        db.alter_column(
            "web_multimedia",
            "url_embed",
            self.gf("django.db.models.fields.CharField")(max_length=400),
        )
        # Adding index on 'Multimedia', fields ['url_embed']
        db.create_index("web_multimedia", ["url_embed"])

        from web.models import Multimedia

        for m in Multimedia.objects.filter(embed_temp__isnull=False):
            m.url_embed = m.embed_temp
            print("%s ==> %s" % (m.url_embed, m.embed_temp))
            m.save()

    def backwards(self, orm):
        # Removing index on 'Multimedia', fields ['url_embed']
        db.delete_index("web_multimedia", ["url_embed"])
