...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        ...
        db.send_create_signal("web", ["PollConditions"])

        # Adding M2M table for field polls on 'PollConditions'
        db.create_table(
            "web_pollconditions_polls",
            (
                (
                    "id",
                    models.AutoField(
                ...
                (
                    "pollconditions",
                    models.ForeignKey(orm["web.pollconditions"], null=False),
                ),
                ("poll", models.ForeignKey(orm["web.poll"], null=False)),
            ),
        )
        db.create_unique("web_pollconditions_polls", ["pollconditions_id", "poll_id"])

    def backwards(self, orm):

        # Deleting model 'PollConditions'
        db.delete_table("web_pollconditions")

        # Removing M2M table for field polls on 'PollConditions'
        db.delete_table("web_pollconditions_polls")
