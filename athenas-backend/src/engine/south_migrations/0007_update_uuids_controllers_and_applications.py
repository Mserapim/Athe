...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        print("UPDATING UUID APPLICATIONS...")
        for app in orm.Application.objects.filter(uuid=None):
            print(app)
            app.uuid = uuid.uuid4().hex
            app.save()
        print("UPDATING UUID CONTROLLER...")
        for ctr in orm.Controller.objects.filter(uuid=None):
            print(ctr)
            ctr.uuid = uuid.uuid4().hex
            ctr.save()
