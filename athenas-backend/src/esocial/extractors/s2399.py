...
class S2399Extractor(Extractor):
    ...
    def __init__(self, instance_outside, *args, **kwargs):
        super(S2399Extractor, self).__init__(instance_outside, *args, **kwargs)
    ...
    def pre_validate(self):
        if (
            ...
            self._instance_outside.servidor.termination_date
            and self._instance_outside.servidor.termination_date
    ...
    def ide_trab_sem_vinculo_cod_categ(self):
        if not self.ide_trab_sem_vinculo_matricula():
            return employee_cod_categ(self._instance_outside.servidor)
        return None

    def info_tsv_termino_mtv_deslig_tsv(self):
        if employee_cod_categ(self._instance_outside.servidor) == "99":
            return 99
        return None
    ...
    def remuntr_empr_tp_insc(self):
        return None

    def remuntr_empr_nr_insc(self):
        return None

    def remuntr_empr_cod_categ(self):
        return None

    def remuntr_empr_vlr_remun_oe(self):
        return None
...
"""
IMPLEMENTAÇÃO ABAIXO NÃO É MAIS REQUERIDA APÓS MUDANÇA DE OBRIGATORIEDADE do campo verbasResc.
"""


class S2399Factory(Factory):

    EXTRACTED_MODEL_CLASS = S2399
    EXTRACTOR = S2399Extractor
    ...
    def _query_instances_outside(cls, **kwargs):
        ...
        query = (
            MovimentacaoDesligamento.objects.filter(
                Q(
                    movimentacao_posse__tipo_movcarreira__in=[
                        "REQUISICAO",
                        "POSSE_ESTAGIARIO",
                        "POSSE_RESIDENTE",
                    ]
        ...
        if extracted_event:
            for to_exc in (
                self.extracted_class.objects.filter(
                    ...
                    ide_trab_sem_vinculo_matricula=instance_outside.servidor.matricula,
                )
