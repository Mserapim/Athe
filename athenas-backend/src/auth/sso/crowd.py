...
class CrowdServer(object):
    ...
    def __init__(
        ...
        self.rest_url = crowd_url + "/crowd/rest/usermanagement/1"
    ...
    def get_session(self, username, password, remote="127.0.0.1", proxy=None):
        ...
        if proxy:
            params["validation-factors"]["validationFactors"].append(
                {
    ...
    def validate_session(self, token, remote="127.0.0.1", proxy=None):
        ...
        if proxy:
            params["validationFactors"].append(
                {
                    "name": "X-Forwarded-For",
                    "value": proxy,
                }
    ...
    def get_user(self, username):
        ...
        response = self._get(
            self.rest_url + "/user",
            params={"username": username, "expand": "attributes"},
        )
    ...
    def remove_child_group(self, name, parent, raise_on_error=False):
        ...
        Returns:
            ...
            error msg (str): If unsuccessful
        """
        params = {"groupname": parent, "child-groupname": name}
        response = self._delete(
            self.rest_url + "/group/child-group/direct", params=params
        )
        if response.status_code == 204:
            return
    ...
    def add_user_to_group(self, username, groupname, raise_on_error=False):
        ...
        response = self._post(
            self.rest_url + "/user/group/direct",
            params={
                "username": username,
            },
            data=json.dumps(data),
    ...
    def send_password_reset_link(self, username):
        ...
        response = self._post(
            self.rest_url + "/user/mail/password", params={"username": username}
        )
    ...
    def get_groups(self, username):
        ...
        response = self._get(
            self.rest_url + "/user/group/direct", params={"username": username}
        )
    ...
    def get_nested_groups(self, username):
        ...
        response = self._get(
            self.rest_url + "/user/group/nested", params={"username": username}
        )
    ...
    def get_nested_group_users(self, groupname):
        ...
        response = self._get(
            self.rest_url + "/group/user/nested",
            params={"groupname": groupname, "start-index": 0, "max-results": 99999},
        )
    ...
    def get_memberships(self):
        ...
        for mg in xmltree.findall("membership"):
            ...
            group = "{}".format(mg.get("group"))
            users = [
                "{}".format(u.get("name")) for u in mg.find("users").findall("user")
            ]
            groups = [
                "{}".format(g.get("name")) for g in mg.find("groups").findall("group")
            ]
            memberships[group] = {"users": users, "groups": groups}
        return memberships
    ...
    def search(
        ...
        response = session.post(
            self.rest_url + "/search", params=params, data=payload, timeout=self.timeout
        )
