...
class ProtocolOnlineTestCase(TestCase):
    ...
    def setUp(self):
        user_perm = UserPermission.objects.get(
            application__title="Site client", user__username="web"
        )

        credentials = ":".join([user_perm.application.app_key, user_perm.user_token])
        headers = {"HTTP_AUTHORIZATION": credentials}

        self.api_uri = "/athenas/OnlineProtocol"
    ...
    def _base_protocol_params(self):
        return {
            "subject": "Denúncia",
            "channel_slug": "protocolo-online",
            "text": """
    ...
    def _anonymous_protocol_params(self):
        params = self._base_protocol_params()
        params.update(person_kind=0, person="ouvidoria-anonimo")
        return params

    def test_create_anonymous_protocol(self):

        endpoint = self._endpoint("anonymous_protocol")
        params = self._anonymous_protocol_params()
        response = self.client.post(endpoint, params)

        self._test_response_requirements(response)

        self.assertEqual(self.json_loaded["message"], "Protocolo realizado.")
        self.assertIn("protocol", self.json_loaded)
        self.assertTrue(self.json_loaded["protocol"])

    def test_create_anonymous_protocol_with_file_upload(self):
    ...
    def _person_base_protocol_params(self):
        params = self._base_protocol_params()
        params.update(
            {
                "address_type": 1,
    ...
    def _citizen_protocol_params(self):
        params = self._person_base_protocol_params()
        params.update(
            {
                "cpf": "261.881.314-64",
                "genre": "M",
    ...
    def test_create_citizen_protocol(self):

        endpoint = self._endpoint("citizen_protocol")
        params = self._citizen_protocol_params()
        response = self.client.post(endpoint, params)
        ...
        self.assertEqual(address.cep, clean_text(params["zipcode"]))

        phone = person.phone.first()
        self.assertEqual(phone.numero, clean_text(params["phone_number"]))

    def test_create_legal_person_protocol(self):

        endpoint = self._endpoint("legal_person_protocol")
        params = self._person_base_protocol_params()
    ...
    def test_load_personal_data(self):
        ...
        params = self._citizen_protocol_params()
        self.client.post(self._endpoint("citizen_protocol"), params)

        response = self.client.get("%s?cpf=%s" % (endpoint, clean_text(params["cpf"])))
        self._test_response_requirements(response)
        self._test_response_data_x_params(params, self.json_loaded["data"])

        params = self._person_base_protocol_params()
        params.update(cnpj="23.716.874/0001-45")
        self.client.post(self._endpoint("legal_person_protocol"), params)

        response = self.client.get(
            "%s?cnpj=%s" % (endpoint, clean_text(params["cnpj"]))
        )
        self._test_response_requirements(response)
        self._test_response_data_x_params(params, self.json_loaded["data"])
