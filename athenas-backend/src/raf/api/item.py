...
from raf.models import Item
from . import util

log = getLogger(__name__)


class RAFItem(RestfulDRY):
    ...
    def json(self, args=[]):
        ...
        self.response.write('Ext._create("raf.item.Launcher")')

    def model_to_dict(self, instance):
        _dict_ = super(RAFItem, self).model_to_dict(instance)
    ...
    def enable(self, args=[]):
        ...
        try:
            item = self.get_query().get(pk=args[0])
            item.activated = not item.activated
            item.save()
            ...
            rst.update(success=True, message="Status modificado")
