...
from judicial.models import LegalClassification, LegalMoviment
from raf.models import Activity, SubItem, TaxonomyClassification
...
class RAFLegalMovement(EJudLegalClassification):

    _model = LegalMoviment
    ...
    def get_listmovements(self, args=[]):
        rst = {
            "success": False,
        ...
        try:
            ...
            subitem = activity.subitem
            legalmoviments = []
            excludemoviments = []
            for lc in TaxonomyClassification.objects.filter(
                subitem=subitem, classification__isnull=False
            ):
                legalmoviments = (
                    legalmoviments
                    + self.create_down_path_legalclassification(
                        lc.cnmp_code, "legalmoviment"
                    )
                )
            for le in TaxonomyClassification.objects.filter(
                subitem=subitem, exclude_classification__isnull=False
            ):
                excludemoviments = (
                    excludemoviments
                    + self.create_down_path_legalclassification(
                        le.cnmp_code, "legalmoviment"
                    )
                )
            lista = []
            if keyword:
                lista = (
                    LegalMoviment.objects.filter(pk__in=legalmoviments)
                    .exclude(pk__in=excludemoviments)
                    .filter(
                ...
                lista = LegalMoviment.objects.filter(pk__in=legalmoviments).exclude(
                    pk__in=excludemoviments
                )
            if lista.count() == 0:
                lista = LegalMoviment.objects.all()
            data = []
            data = [
                {
                    "pk": a.pk,
