...
from raf.models import Activity, Item, TaxonomyClassification
...
class RAFLegalMatter(EJudLegalClassification):
    ...
    def create_down_path_legalclassification(self, classification_code):
        retorno = []
        while classification_code is not None:
            classification = LegalClassification.objects.filter(
                cnmp_code=classification_code
            ).first()
            if classification:
    ...
    def get_listmatters(self, args=[]):
        ...
        try:
            ...
            activity = Activity.objects.filter(pk=params.get("activity")).first()
            item = activity.item
            legalmatters = []
            excludematters = []
            for lc in TaxonomyClassification.objects.filter(
                item=item, classification__isnull=False
            ):
                legalmatters = legalmatters + self.create_down_path_legalclassification(
                    lc.cnmp_code
                )
            for le in TaxonomyClassification.objects.filter(
                item=item, exclude_classification__isnull=False
            ):
                excludematters = (
                    excludematters
                    + self.create_down_path_legalclassification(le.cnmp_code)
                )
            lista = []
            if keyword:
                lista = (
                    LegalClassification.objects.filter(pk__in=legalmatters)
                    .exclude(pk__in=excludematters)
                    .filter(
                ...
                lista = LegalClassification.objects.filter(pk__in=legalmatters).exclude(
                    pk__in=excludematters
                )
            if lista.count() == 0:
                lista = LegalClassification.objects.exclude(
