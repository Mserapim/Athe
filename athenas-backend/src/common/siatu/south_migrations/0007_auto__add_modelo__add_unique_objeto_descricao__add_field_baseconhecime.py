...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        from common.siatu.models import BaseConhecimento, Objeto, Modelo
        ...
        db.send_create_signal("siatu", ["Modelo"])

        # Adding M2M table for field modelos on 'Objeto'
        m2m_table_name = db.shorten_name("siatu_objeto_modelos")
        db.create_table(
            m2m_table_name,
            (
                (
                ...
                ("objeto", models.ForeignKey(orm["siatu.objeto"], null=False)),
                ("modelo", models.ForeignKey(orm["siatu.modelo"], null=False)),
            ),
        )
        db.create_unique(m2m_table_name, ["objeto_id", "modelo_id"])

        # Adding unique constraint on 'Objeto', fields ['descricao']
        db.create_unique("siatu_objeto", ["descricao"])

        # Adding field 'BaseConhecimento.modelo_tmp'
        db.add_column(
        ...
        modelos = [b.modelo for b in BaseConhecimento.objects.all()]
        modelos_new = list(set(modelos))
        modelos_new.sort()

        for m in modelos_new:
            Modelo(descricao=m).save()

        modelo_padrao = Modelo.objects.get(descricao="Padrão")
        for o in Objeto.objects.all():
            o.modelos.add(modelo_padrao)

        for base in BaseConhecimento.objects.all():
            modelo_z = Modelo.objects.get(descricao=base.modelo)
            base.modelo_tmp = modelo_z
            base.save()
            if (
                base.objeto.modelos.filter(descricao=modelo_z.descricao).exists()
                is False
            ):
                base.objeto.modelos.add(modelo_z)

    def backwards(self, orm):
        # Removing unique constraint on 'Objeto', fields ['descricao']
        db.delete_unique("siatu_objeto", ["descricao"])

        # Deleting model 'Modelo'
        db.delete_table("siatu_modelo")

        # Removing M2M table for field modelos on 'Objeto'
        db.delete_table(db.shorten_name("siatu_objeto_modelos"))

        # Deleting field 'BaseConhecimento.modelo_tmp'
        db.delete_column("siatu_base_conhecimento", "modelo_tmp_id")
