...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        from engine.models import Controller, ControllerPermission

        call_command("loaddata", "siatu_Application.json")
        call_command("loaddata", "siatu_Funcionalidades.json")

        [c.save() for c in Controller.objects.filter(controller__icontains="siatu")]

        call_command("loaddata", "siatu_ControllerPermission_Admin.json")
        call_command("loaddata", "siatu_ControllerPermission_Gerentes.json")
        call_command("loaddata", "siatu_ControllerPermission_Atendentes.json")
        call_command("loaddata", "siatu_Group_Admin.json")
        call_command("loaddata", "siatu_Group_Gerentes.json")
        call_command("loaddata", "siatu_Group_Atendentes.json")
        call_command("loaddata", "siatu_TipoAvaliacao.json")
        call_command("loaddata", "siatu_Func_abrirChamado.json")
        call_command("loaddata", "siatu_Messages.json")

        call_command("loaddata", "siatu_objetos.json")

        grupo = Group.objects.get(name="Basico")
        permission1 = Permission.objects.get_by_natural_key(
            "add_solicitacao", "siatu", "solicitacao"
        )
        permission2 = Permission.objects.get_by_natural_key(
            "change_chamado", "siatu", "chamado"
        )
        permission3 = Permission.objects.get_by_natural_key(
            "add_avaliacao", "siatu", "avaliacao"
        )
