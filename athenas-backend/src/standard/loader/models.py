...
class FileLoader(object):
    ...
    @property
    def get_name_return_file(self):
        split_file = os.path.splitext(self.file)
        return "%s_return_file.ret%s.txt" % split_file

    def create_return_file(self):
        ret_file = self.get_name_return_file
        with codecs.open(ret_file, "w", self.CODE_TYPE) as f:
            ...
            for obj in self.objects:
                ...
                if self.TRUNK_CR_NL:
                    line = "%s%s" % (line, "\r\n")
                f.write(line)
        return Arquivo.from_filepath(
            os.path.join(ret_file), get_current_user(), "application/txt"
        )
