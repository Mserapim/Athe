...
def enviar_email_movimentacao_fluxo_beneficiario(beneficiario):
    try:
        email_template_code = "DIARIAS_MUDANCA_FLUXO"
        ...
        message = (
            ...
            .replace(
                "%ETAPA_ANTIGA%",
                f"{fluxos[1].fluxo.get_situacao_display()} - {fluxos[1].fluxo.get_etapa_display()}",
            )
            .replace(
                "%ETAPA_NOVA%",
                f"{fluxos[0].fluxo.get_situacao_display()} - {fluxos[0].fluxo.get_etapa_display()}",
            )
        )
        ...
        log.info(
            f"Envio de notificação de mudança de etapa da solicitação de diaria do beneficiario {beneficiario.servidor.pessoa_fisica.social_name} - {beneficiario.codigo_os}"
        )
        print(
            f"Envio de notificação de mudança de etapa da solicitação de diaria do beneficiario {beneficiario.servidor.pessoa_fisica.social_name} - {beneficiario.codigo_os}"
        )
