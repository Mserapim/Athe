...
from rh.task.sicapap import sicap_maker
...
class RHSicapApGenerator(DefaultController):
    ...
    def start(self, args=[]):
        ...
        paymentfile_ = (
            File.objects.get(pk=self.request.POST.get("paymentfile")).pk
            if self.request.POST.get("paymentfile")
            else None
        )
        try:
            Task.start(
    ...
    <p>Este arquivo está disponível para download até dia <span style="font-weight:bold">%(deadline)s</span></p>""",
            ...
            rst.update(
                ...
                % (
                    SicapHelper._months_to_unicode(
    ...
    def file(self, args=[]):
        cache_path = SicapHelper.directory_tmp()
        try:
