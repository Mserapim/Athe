...
class Differences(BaseCalculation):
    ...
    def value(self):
        ...
        if self.difference:
            if (
                ...
                value = self.config_to_next_installment.value
            else:
    ...
    def installment(self):
        ...
        # log.debug(u'PORCENTAGEM de BaseCalculo: %s' % installment)
        return installment
    ...
    def total_installment(self):
        total_installment = 0
        # log.debug('TI: %s DIFF: %s' % (self.params.get('total_installment'), self.difference))
        if self.difference and self.difference.installments > 1:
            # if self.config_to_next_installment.typeof in [2, 3]:
            # log.debug('TI >> %s: %s' % (self.payable, self.value()))
            total_installment = math.ceil(abs(float(self.payable) / self.value()))
