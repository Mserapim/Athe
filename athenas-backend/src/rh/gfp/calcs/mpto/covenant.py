...
@RunCodeManager.register("gfp-mpto-sindsemp")
...
@RunCodeManager.register("gfp-mpto-fenasemp")
...
@RunCodeManager.register("gfp-mpto-asamp")
...
@RunCodeManager.register("gfp-mpto-atmp")
...
@RunCodeManager.register("gfp-mpto-atmp-13salario")
    ...
    def quantity(self):
        if self.employee.posses.order_by("-data_exercicio").exists():
            ...
            if posse.data_exercicio.year < self.payroll.periodo.ano:
                ...
                # log.debug('O servidor %s não tem direito a 13° para %s' % (self.employee.pessoa_fisica, self.payroll))
                r = 0.00
        else:
            # log.debug('O servidor %s servidor não tem posse.' % self.employee.pessoa_fisica)
            r = 0.00
...
@RunCodeManager.register("gfp-mpto-plansaude-monthly-payment")
