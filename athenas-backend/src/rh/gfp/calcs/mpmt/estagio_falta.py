...
@RunCodeManager.register("gfp-mpmt-faltabolsaestagio")
class FaltaBolsaEstagio(WorkDaysCalculation):
    title = "Cálculo Base para desconto de Falta(s) de Estagiário na Bolsa"
    ...
    def buscar_fe_bolsa_estagio(self, cc):
        q_fe = cc.lancamentos.filter(evento__numero="07500")  # Bolsa Estágio
    ...
    #         event__numero='07500', # Bolsa Estágio
...
@RunCodeManager.register("gfp-mpmt-faltatransporteestagio")
class FaltaTransporteEstagio(WorkDaysCalculation):
    title = "Cálculo Base para desconto de Falta(s) de Estagiário no Aux. Transporte"
    ...
    def buscar_fe_transporte_estagio(self, cc):
        q_fe = cc.lancamentos.filter(evento__numero="13500")  # Transporte Estágio

        return q_fe.first() if q_fe.exists() else None

    # def buscar_config_evento_vigente(self, dt_inicio, dt_fim):
    #     q_config_event = ConfigEvent.objects.filter(
    #         event__numero='13500', # Transporte Estágio
    #         start_validity__lte=dt_fim,
    #     ).filter(
    #         Q(end_validity__isnull=True) |
    #         Q(end_validity__gte=dt_inicio)
