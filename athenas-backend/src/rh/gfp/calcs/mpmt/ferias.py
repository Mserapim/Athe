...
from rh.gfp.calcs.mpmt.remuneracao import BaseSalary
from rh.gfp.calcs.mpmt.base import BaseCalculation
from rh.models import Dependencia, ProcessSuspension
from rh.gfp.models import IRRF as IRRFModel, FolhaEvento
...
@RunCodeManager.register("gfp-mpmt-additional-vacation")
...
@RunCodeManager.register("gfp-mpmt-additional-vacation-ii")
...
@RunCodeManager.register("gfp-mpmt-unused-vacation")
...
@RunCodeManager.register("gfp-mpmt-unused-vacation-ii")
...
@RunCodeManager.register("gfp-mpmt-additional-unused-vacation")
...
@RunCodeManager.register("gfp-mpmt-proportional-vacation")
...
@RunCodeManager.register("gfp-mpmt-proportional-vacation-ii")
...
@RunCodeManager.register("gfp-mpmt-additional-proportional-vacation")
...
@RunCodeManager.register("gfp-mpmt-irrf-vacation")
class IRRFVacation(BaseCalculation):
    title = "Calculo de férias de imposto retido na fonte"
    ...
    def __init__(self, employee, payroll, event=None, entry=None, cid=None, **kwargs):
        ...
        """
        super(IRRFVacation, self).__init__(
            employee, payroll, event, entry, cid=cid, **kwargs
        )
        if self.suspension:
            self.exclude_events += [
    ...
    def validate_unique_IRPF(self):
        if not FolhaEvento.objects.filter(
            folha=self.payroll,
            ...
            raise Exception(
                "A verba somente pode ser aplicada na mesma folha onde a verba de férias foi implementada."
            )

    def validate(self):
        self.validate_unique_IRPF()
    ...
    def suspension(self):
        return ProcessSuspension.objects.filter(
            ...
            & Q(
                ...
                end_validity__lte=self.range_salary.first,
            )
        ).first()
    ...
    def discount_sixty_five_years_inactive(self):
        value_discount = 0
        if self.employee.type_by_possession in ("SAP", "MAP", "MAP2"):
...
@RunCodeManager.register("gfp-mpmt-irrf-vacation-diff")
class IRRFVacationDifference(IRRFVacation):
    title = "Calculo de férias de imposto retido na fonte - diferença"


@RunCodeManager.register("gfp-mpmt-irrf-vacation-devolution")
class IRRFVacationDevolution(IRRFVacation):
    title = "Calculo de férias de imposto retido na fonte - devolução"


@RunCodeManager.register("gfp-mpmt-irrf-vacation-prev-months")
class IRRFVacationPrevMonths(IRRFVacation):
    title = "Calculo de férias de imposto retido na fonte - meses anteriores"


@RunCodeManager.register("gfp-mpmt-irrf-vacation-chargeback-debit")
class IRRFVacationChargebackDebit(IRRFVacation):
    title = "Calculo de férias de imposto retido na fonte - estorno débito"


@RunCodeManager.register("gfp-mpmt-irrf-vacation-chargeback-credit")
class IRRFVacationChargebackCredit(IRRFVacation):
    title = "Calculo de férias de imposto retido na fonte - estorno crédito"
