...
class ProvisionGenerator(object):
    ...
    class CreatePeriodsNotImplemented(Exception):
        def __init__(self):
            Exception.__init__(
                self,
                'ERRO - Informe os desenvolvedores do sistema: O método "create_periods" deve ser implementado\
                na classe especializada, para criar os períodos(provisions) de cada servidor',
            )
...
@RunCodeManager.register("gfp-classcodes-provision-socialsecurity-christmas")
    ...
    def create_period_for_employee(self, provision_manager, employee, task=None):
        ...
        if (
            ...
            while dt_rel.month <= provision_manager.reference_month:
                # log.debug(u'AAAA %s %s %s' % (daterange_acquisition, daterange_rel, dt_while))
