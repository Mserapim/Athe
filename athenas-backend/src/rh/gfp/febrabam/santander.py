...
__name__ = "Banco Santander (Brasil) S.A."
__hid__ = "033"
...
class File(Protocol):
    ...
    def __init__(self, conf):
        ...
        config_layout = {
            ...
            "nome_banco": "Banco Santander Banespa",
            "arquivo_densidade": 0,
        }
                ...
                if dbs.count() and dbs[0].dado_bancario_pessoa.banco == self.banco:
                    ...
                    if (credito) > 0.001:
                        ...
                        lote.addRegistro(
                            ...
                            favorecido_camara=0,  # 18 if (positivo - negativo)>= 3000 else 700,
                            ...
                            credito_data_real=conf["folha"].dt_pagamento.strftime(
                                "%d%m%Y"
                            ),
                        )
                        soma += credito
        ...
        if self.banco.principal:
            ...
            for info in list_servidores_doc:
                ...
                for e in query_folhaeventos:
                    evento = e.folha_evento.evento if info.get("pensao") else e.evento
                    credito = (
                        (credito + float(e.valor))
                        if evento.tipo == "P"
                        else (credito - float(e.valor))
                ...
                if (credito) > 0.001:
                    ...
                    lote2.addRegistro(
                        ...
                        credito_valor_pgto=credito,
                        credito_data_real=conf["folha"].dt_pagamento.strftime("%d%m%Y"),
                    )
                    lote2.addRegistro(
                        ...
                        comple_pagamento_valor=credito,
                        comple_pagamento_vencimento=conf["folha"].dt_pagamento.strftime(
                            "%d%m%Y"
                        ),
                    )
                    soma += credito
