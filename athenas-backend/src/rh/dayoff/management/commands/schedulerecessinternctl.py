...
class Command(BaseCommand):
    verbose = "False"
    help = """
        Este comando irá executar uma rotina mensal que notifica e agenda o recesso de estagiários
    """
    ...
    def handle(self, *args, **options):
        self.notify_schedule_recess()
        self.intern_unscheduled_recess()

    def notify_schedule_recess(self):
        self.conf()
        date = datetime.now()
        print(
            f">>> [{DateUtils.datetime_to_str(date)}] Iniciando rotina de notificação de recesso de estagiários >>>>>>>>>>>>>"
        )
        log.info(
            f">>> [{DateUtils.datetime_to_str(date)}] Iniciando rotina de notificação de recesso de estagiários >>>>>>>>>>>>>"
        )
        ...
        for acq_period in (
            ...
            try:
                ...
                # teste_date = datetime(2023,5,5).date()
                ...
                if date.date() in [
                    ...
                    # Envio de notificação para o estagiário
                    intern_template_email = EmailTemplate.objects.get(
                        code="NOTIFICAO_RECESSO_ESTAGIARIO"
                    )
                    intern_receivers = [
                        {
                    ...
                    # Envio de notificação para supervidor do estágio
                    supervidor_template_email = EmailTemplate.objects.get(
                        code="SUPERVISOR_NOTIFICAO_RECESSO_ESTAGIARIO"
                    )
                    ...
                    contents = supervidor_template_email.contents.replace(
                        "%data%", dt_recess.strftime("%d/%m/%Y")
                    ).replace("%estagiario%", employee.pessoa_fisica.nome)
                    EmailNotification().send_email_default(
        ...
        log.info(
            ">>> [%s] Finalizando rotina de notificação de recesso de estagiários >>>>>>>>>>>>>"
    ...
    def intern_unscheduled_recess(self):
        self.conf()
        date = datetime.now()
        print(
            f">>> [{DateUtils.datetime_to_str(date)}] Iniciando rotina de agendamento de recesso de estagiários nos último 30 dias >>>>>>>>>>>>>"
        )
        log.info(
            f">>> [{DateUtils.datetime_to_str(date)}] Iniciando rotina de agendamento de recesso de estagiários nos último 30 dias >>>>>>>>>>>>>"
        )
        ...
        for acq_period in AcquisitionPeriod.objects.filter(
            ...
            try:
                ...
                diffs = date.date() - dt_exercise
                qtd_months = diffs.days // 30
                # log.info(acq_period.end_date_fruition - dt_exercise)
                if qtd_months in [15, 23]:
                    ...
                    if (
                        ...
                        template_email = EmailTemplate.objects.get(
                            code="AGENDAMENTO_RECESSO_ESTAGIARIO"
                        )
                        receivers = [
                            {
                                "email": employee.pessoa_fisica.email_institucional,
        ...
        log.info(
            ">>> [%s] Finalizando rotina de agendamento de recesso de estagiários nos último 30 dias >>>>>>>>>>>>>"
            % DateUtils.datetime_to_str(date)
        )
        print(
            ">>> [%s] Finalizando rotina de agendamento de recesso de estagiários nos último 30 dias >>>>>>>>>>>>>"
