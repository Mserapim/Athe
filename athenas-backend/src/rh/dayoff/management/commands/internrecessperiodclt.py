...
class Command(BaseCommand):
    ...
    help = """
        ...
        proximo periodo aquisitivo para proximo recesso de estagiários
    """
    ...
    def handle(self, *args, **options):
        self.intern_recess_period()

    def intern_recess_period(self):
        self.conf()
        date = datetime.now()
        print(
            f">>> [{DateUtils.datetime_to_str(date)}] Iniciando rotina de agendamento de recesso de estagiários >>>>>>>>>>>>>"
        )
        log.info(
            f">>> [{DateUtils.datetime_to_str(date)}] Iniciando rotina de agendamento de recesso de estagiários >>>>>>>>>>>>>"
        )
        ...
        log.info(
            ">>> [%s] Finalizando rotina de agendamento de recesso de estagiários >>>>>>>>>>>>>"
