...
from corregedoria.prontuary.models import DetailExercise
import raf.api.util

log = getLogger(__name__)


class PRONTUARYDetailExercise(RestfulDRY):

    force_upper = False

    full_text_index = ()

    _model = DetailExercise
    ...
    def model_to_dict(self, instance):
        _dict_ = super(PRONTUARYDetailExercise, self).model_to_dict(instance)
        _dict_.update(
            {
    ...
    def renderer_document(self, args=[]):
        ...
        try:
            ...
            detailexercise = DetailExercise.objects.filter(
                pk=int(params.get("detailexercise", 0) or 0)
            ).first()
            ...
            rst.update(success=True, content=detailexercise.rendered)
        self.renderer(rst)
