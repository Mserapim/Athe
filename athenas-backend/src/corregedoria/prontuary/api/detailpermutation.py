...
from corregedoria.prontuary.models import DetailPermutation
import raf.api.util

log = getLogger(__name__)


class PRONTUARYDetailPermutation(RestfulDRY):

    force_upper = False

    full_text_index = ()

    _model = DetailPermutation
    ...
    def model_to_dict(self, instance):
        _dict_ = super(PRONTURYDetaildPermutation, self).model_to_dict(instance)
        _dict_.update(
            {
    ...
    def renderer_document(self, args=[]):
        ...
        try:
            ...
            detailpermutation = DetailPermutation.objects.filter(
                pk=int(params.get("detailpermutation", 0) or 0)
            ).first()
            ...
            rst.update(success=True, content=detailpermutation.rendered)
        self.renderer(rst)
