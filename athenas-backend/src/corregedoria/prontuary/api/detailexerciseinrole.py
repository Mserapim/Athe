...
from corregedoria.prontuary.models import DetailExerciseInRole
import raf.api.util

log = getLogger(__name__)


class PRONTUARYDetailExerciseInRole(RestfulDRY):

    force_upper = False

    full_text_index = ()

    _model = DetailExerciseInRole
    ...
    def model_to_dict(self, instance):
        _dict_ = super(PRONTUARYDetailExerciseInRole, self).model_to_dict(instance)
        _dict_.update(
            {
    ...
    def renderer_document(self, args=[]):
        ...
        try:
            ...
            detailexerciseinrole = DetailExerciseInRole.objects.filter(
                pk=int(params.get("detailexerciseinrole", 0) or 0)
            ).first()
            ...
            rst.update(success=True, content=detailexerciseinrole.rendered)
        self.renderer(rst)
